...
class PortToReceive(threading.Thread):
    ...
    def run(self):
        self._running = True
        try:
            with serial.Serial(self.port, exclusive=True, timeout=0.001) as ser:
                while self._running:
                    if ser.in_waiting:
                        data = ser.readall().decode('utf-8')
                        print(f"READ {data}")
                        self.data_callback(data)
                    time.sleep(0.1)
        except serial.SerialException as e:
            self.handle_port_error(True)
            self.port = None
            self._running = False
    ...
    def write(self, package):
        if self._running and self.ser and self.ser.is_open:
            try:
                ...
                self.ser.write(package.encode('utf-8'))  # Отправка сообщения
